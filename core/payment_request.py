...
# Search Account you want to request fund from using the inputed Account Number or Account ID
@login_required
def SearchUsersRequest(request):
    account = Account.objects.all() # You can choose to get all accounts or filter by the active accounts only
    query = request.POST.get("account_number") ## <input name="account_number"> From the input, get the searched account number

# Use Q to filter through all given accounts and search for the requested the account using the ACCOUNT NUMBER or ACCOUNT ID
    if query:
...
#  Get the user account info of the ACCOUNT NUMBER of the user you want to send a request to
...
# Create a new TRTANSACTION form the amount to be requested
...
# Confirm PIN is correct and create a notification for the request
